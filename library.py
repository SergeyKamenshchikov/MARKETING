...
##FOREST##
def randomforest(X_features,z):
  T2=get_class(z)

  scores=[]
  for item in range(2,cv_n):
    rf=RandomForestRegressor(n_estimators=forest_max,criterion='mse', random_state=1,max_features=None)
    cv_scores=list(cross_val_score(rf,X_features,T2,cv=item,scoring='roc_auc'))

  return ([outlist(cv_scores)]+prediction(X_features,T2,rf))
...
def mlp(X_features,z):
  ...
  scores=[]
  for item in range(2,cv_n):
    param_grid = {'max_iter': np.arange(10,iterations), 'alpha':[0.1,0.01,0.001,0.0001]}
    clf=MLPClassifier()
    clf_cv=GridSearchCV(clf, param_grid, scoring='roc_auc', cv=item)
    clf_cv.fit(X_features,T3)
    iter_opt=clf_cv.best_params_['max_iter']
    alpha_opt=clf_cv.best_params_['alpha']

    clf_final=MLPClassifier(activation='relu', hidden_layer_sizes=(1,1), alpha=alpha_opt, max_iter=iter_opt, random_state=1, solver='lbfgs')
