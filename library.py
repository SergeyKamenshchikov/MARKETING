...
##FOREST##
def dtree(X_features,z):
  T2=get_class(z)

  scores=[]
  for item in range(2,cv_n):
    dtree=DecisionTreeClassifier(criterion='entropy')
    cv_scores=list(cross_val_score(dtree,X_features,T2,cv=item,scoring='roc_auc'))

  return ([outlist(cv_scores)]+prediction(X_features,T2,dtree))
...
def mlp(X_features,z):
  ...
  scores=[]
  for item in range(2,cv_n):
    clf_final=MLPClassifier(activation='relu', hidden_layer_sizes=(3,1), max_iter=iterations, random_state=1, solver='lbfgs')
